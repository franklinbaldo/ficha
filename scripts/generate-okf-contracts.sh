...
cd "$ROOT"
OUT_DIR="${OKF_OUT_DIR:-web/src/generated}"
mkdir -p "$OUT_DIR"
...
"${OKF[@]}" schema knowledge --format zod --spec-template "$SPEC_TEMPLATE" "${DATA_ONLY[@]}" > "$OUT_DIR/ficha-okf.zod.ts"
"${OKF[@]}" schema knowledge --format json --spec-template "$SPEC_TEMPLATE" "${DATA_ONLY[@]}" > "$OUT_DIR/ficha-okf.schema.json"
...
uv run --with duckdb python scripts/export-okf-views.py "$TMP_DB" > "$OUT_DIR/ficha-okf.views.ts"
