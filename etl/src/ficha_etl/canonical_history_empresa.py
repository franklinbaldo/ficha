...
invocation -- legitimate there because ``duplicate_policy="fail"``), empresa
declares ``duplicate_policy="deterministic-collapse"``: a ``cnpj_basico``
duplicated across two different physical ``EmpresasN.zip`` parts can only be
collapsed correctly if both parts are in the same deduplication scope (see
``canonical_shadow.py``'s module docstring and ``write_canonical_dataset``).
This orchestrator therefore always resolves the COMPLETE ten-part physical
set for one snapshot before writing anything.
...
def empresa_remotes(month: str) -> list[RemoteFile]:
    ...
    expected = [spec.name for spec in sources.canonical_inventory() if spec.kind == "empresas"]
    if len(expected) != _PART_COUNT:  # pragma: no cover - sources.py invariant
        raise RuntimeError(
            f"expected {_PART_COUNT} empresa parts, sources.py declares {expected!r}"
        )
    return [
...
def _ensure_and_extract_one(
    ...
    retained once extracted. Returns (source manifest entry, extracted CSV
    path)."""
...
def run_historical_empresa_dataset(
    ...
    *,
    ...
    local ZIP path -- smoke/offline runs only. Any remote NOT covered by an
    override is preflight-checked (HEAD) before any download starts; a
    partial real source set fails closed rather than silently proceeding
    with fewer than ten parts.
    """
    if sample_size < 0:
    ...
    try:
        for remote in remotes:
            entry, csv_path = _ensure_and_extract_one(
                remote, paths, overrides.get(remote.name), client
            )
        ...
        manifest: dict[str, Any] = {
            ...
            # Key names mirror metrics.StageMetrics.to_json_dict()'s actual
            # contract exactly (wall_seconds/rss_peak_delta_mib, not
