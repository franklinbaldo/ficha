...
def write_cnpjs_parquet_chunked(
    ...
    try:
        ...
        for i, csv_path in enumerate(estabelecimento_csv_paths):
            ...
            # LZ4, not ZSTD: chunk parts are transient — the merge step below
            # reads them all back, globally sorts by cnpj, and re-compresses the
            # final cnpjs.parquet with ZSTD. ZSTD-encoding these throwaway bytes
            # is wasted CPU; LZ4 is much faster to write and the parts stay on
            # local disk.
            con.execute(
                f"""
                COPY (
