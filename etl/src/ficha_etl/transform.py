...
def write_cnpj_contatos_parquet(
    ...
    # One scan, not four. The old query UNION-ALL'd four full scans of
    # estabelecimento (telefone_1 / telefone_2 / fax / email). A single
    # correlated LATERAL VALUES fans each row into its ≤4 candidate contacts,
    # then WHERE drops the empty ones — identical output (verified row-for-row),
    # a quarter of the scans. `nullif(x,'') || nullif(y,'')` yields NULL when
    # either side is empty/NULL (the `||` operator propagates NULL), reproducing
    # the old per-branch "ddd AND numero both non-empty" guard.
    con.execute(
        """
        COPY (
...
def write_cnpj_cnaes_parquet(
    ...
    # One scan, not two. The old query scanned estabelecimento twice (primary
    # CNAE, then a LATERAL-unnested secondary list) and UNION-ALL'd them. Here
    # a single scan builds one list per row — the primary at posicao 0 followed
    # by the comma-split secondaries at their 1-based positions (list_transform's
    # index reproduces the old generate_subscripts idx) — and UNNEST expands it.
    # Output is identical row-for-row (verified), including empty split parts,
    # which the old query also emitted.
    con.execute(f"""
        COPY (
          WITH _c AS (
            SELECT
