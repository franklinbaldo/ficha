...
uv run python bench/ab_verify_roundtrip.py --repeats 5 --sample-size 1000
```
...
| No record of what configuration/machine actually ran | `bench/_profile.py:capture_environment()` — DuckDB version, effective threads/memory_limit/preserve-insertion-order/temp-directory (read back via `current_setting`, not just what was requested), platform, CPU count, git SHA, harness version, in every JSON result |
...
| `bench/` wasn't linted or run by CI — a schema-registry refactor silently broke all three scripts (removed columns they still referenced) while lint+tests stayed green | `ci.yml`'s `etl` job now lints `bench/` too, and runs all three original harness scripts as a smoke check |
...
The roundtrip verifier A/B adds one more conservative control: both historical
and current variants use the exact same reservoir sample and fingerprint. The
historical side keeps its one-Parquet-query-per-CNPJ shape but is spared its old
`ORDER BY random()` full-table sort, so the measured advantage of the current
single-query verifier cannot be attributed to different sampling work.
...
- [`evidence/2026-07-20-roundtrip-verifier.md`](evidence/2026-07-20-roundtrip-verifier.md)

They record commit/run provenance, environment, medians, spread, correctness and
resource measurements. The resulting production decisions are:
...
| Historical multi-query roundtrip verifier | Current verifier is 99.2× faster; both pass valid and reject corrupted output | Keep current single-query verifier |
...
These pre-methodology-review runs are preserved as history. The current
production-profile results supersede their decision status.

| Change | Exploratory result (old harness) | Current status |
|---|---|---|
| Single-query `reservoir REPEATABLE` roundtrip verification | 23.4s → 1.1s at 300k (21×) | Confirmed under production profile: 14.8044s → 0.1492s at 500k (99.2×) |
...
All recorded Fase 0 performance questions now have decision-grade evidence. The
benchmark-measurement phase is closed; the next work is the first RFC delivery:
canonical registry → canonical estabelecimento → historical shadow workflow →
triangular raw/canonical/product validation.
